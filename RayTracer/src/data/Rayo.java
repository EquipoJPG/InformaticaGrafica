...
public class Rayo {
	...
	public static Rayo rayoRefractado(Rayo vista, Objeto o, Vector4 i, double eps) {
		...
		double titaT = Math.acos(Math.sqrt(1 - Math.pow(nR, 2) * (1 - Math.pow(Vector4.dot(vistaNegada, normal), 2))));
		double titaI = Vector4.dot(vistaNegada, normal);
		
		Vector4 primerTerm = Vector4.mulEscalar(normal, (nR * Math.cos(titaI)) - Math.cos(titaT));
